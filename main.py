...
def execute_tool(name: str, inp: dict) -> dict:
    ...
    if name == "add_task":
        ...
        # Проверяем дубли ПЕРЕД добавлением
        existing, _ = gtasks_list(show_completed=False)
        title_lower = inp["title"].strip().lower()
        for t in existing:
            if t.get("title", "").strip().lower() == title_lower:
                return {"ok": False, "duplicate": True,
                        "message": f"Задача '{inp['title']}' уже есть в Google Tasks — не добавляю дубль."}
